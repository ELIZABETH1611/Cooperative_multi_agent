...
class pathfinding(object):
    ...
    def __init__(self,laser_angles,x_min=-12,x_max=12,y_min=-12,y_max=12,size_s=0.075,debug=False):
        ...
        if debug:
            dummy = self.box
            dummy[0,0]=2
            self.__debug_im = plt.imshow(dummy,extent=(self.x_min,self.x_max,self.y_max,self.y_min),vmin=0,vmax=3)
    ...
    def __init_map(self,x_min,x_max,y_min,y_max,size_s):
        ...
        self.robot_map       = np.zeros([len(self.x_coordinates),len(self.y_coordinates)])
        ...
        self.tmp_map         = np.zeros([len(self.x_coordinates),len(self.y_coordinates)])
    ...
    def monitor(self,robot_pos,target_pos):
        ...
        if self.__debug:
            try:
                tmp_map=self.tmp_map.copy()
                tmp_map[self.robot_map==2]=3
                self.__debug_im.set_array(tmp_map)
    ...
    def __a_star(self,map,start,goal,eps=0.5):
        """
        ...
        while len(openset)>0:
            ...
            if abs(time.time()-time_initial)>1.5:
                return goal
    def reset_avoid_robot_map(self):
        self.robot_map = np.zeros(self.box.shape)
    def avoid_robot_map(self,robot_x_n, robot_y_n, goal_x_n, goal_y_n,own_path,n_steps=10):
        v_l=0.15 #linear velocity
        # map_tmp=np.zeros(self.box.shape)
        # map_tmp[self.box_free>=1]=1
        # map_tmp[self.box_wall/self.box_free >7]=2
        map_tmp = self.tmp_map.copy()
        map_tmp[self.robot_map>=2]=2
        robot_position  = np.array([robot_x_n,robot_y_n])
        target_position = np.array([goal_x_n,goal_y_n])
        # Get the position of the robot and target in box coordinates
        robot_position  = np.array([np.argmin(abs(self.x_coordinates-robot_position[0])),np.argmin(abs(self.y_coordinates-robot_position[1]))])
        target_position = np.array([np.argmin(abs(self.x_coordinates-target_position[0])),np.argmin(abs(self.y_coordinates-target_position[1]))])
        for i in range(1,5):
            map_tmp[target_position[0]-i,target_position[1]] = 0
            map_tmp[target_position[0]+i,target_position[1]] = 0
            map_tmp[target_position[0],target_position[1]-i] = 0
            map_tmp[target_position[0],target_position[1]+i] = 0
        map_tmp[robot_position[0],robot_position[1]] = 0
        print(robot_x_n, robot_y_n, goal_x_n, goal_y_n,own_path)
        try:
            path_robot_n= self.__a_star(map_tmp.T,tuple(robot_position),tuple(target_position))
            path_robot_n=tuple(path_robot_n[-n_steps:,::-1].T)
            print(path_robot_n[:n_steps])
            if not own_path:
                self.robot_map[path_robot_n]=2
            else:
                self.robot_map[path_robot_n]=5
            self.robot_map[1:,:]=self.robot_map[1:,:]+self.robot_map[:-1,:]
            self.robot_map[(self.robot_map==4) | (self.robot_map==7)]=2
            self.robot_map[:,1:]=self.robot_map[:,1:]+self.robot_map[:,:-1]
            self.robot_map[(self.robot_map==4) | (self.robot_map==7)]=2
            # self.robot_map=self.robot_map.T
        except:
            return
